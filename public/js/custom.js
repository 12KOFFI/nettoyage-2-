...
$(function () {
  ...
  // Navbar scrolling background - OPTIMIZED
  ...
  window.addEventListener('scroll', handleNavbarScroll, window.scrollOptimizer.scrollOptions);
  ...
  class Cursor {
    ...
    addEvents() {
      window.addEventListener("mousemove", this.getMousePosition, { passive: true });
    }
    on() {
      this.addEvents();
    }
  ...
  if ($(".js-cursor").length > 0 && window.innerWidth > 1024) {
    ...
    // cursor conditions
    const dragItems =
      ".slider-fade .owl-theme .item, .services1 .owl-theme .item, .services2 .owl-theme .item, .team .owl-theme .item, .testimonials .owl-theme .item, .blog .owl-theme .item";
    $(dragItems)
